...
const server = {
  ...
  init: async function() {
    ...
    try {
      ...
      // deploy first token
      const token1Contract = await this.deployDummyToken(100000000000, 'Tronix', 6, 'TRX')
      console.log(`First token TRX has been deployed at ${token1Contract.address}`)

      // deploy second token
      const token2Contract = await this.deployDummyToken(	1000000000, 'EOS', 18, 'EOS')
      console.log(`Second token EOS has been deployed at ${token2Contract.address}`)

      // deploy third token
      const token3Contract = await this.deployDummyToken(197192382, 'Binance Coin', 18, 'BNB')
      console.log(`Third token BNB has been deployed at ${token3Contract.address}`)

      // deploy fourth token
      const token4Contract = await this.deployDummyToken(1000000000, 'VeChain', 18, 'VEN')
      ...
      await token1Contract.transfer(addrArray[1], 50000000000, { from: `${this.address}`, gas: 5000000 })
      console.log(`50000000000 TRX successfully transferred to ` + addrArray[1])

      await token1Contract.transfer('0x9fa877509a7b5d90bdbdf911aeeaacce12857521', 50000000000, { from: `${this.address}`, gas: 5000000 })
      ...
      await token2Contract.transfer(addrArray[1], 500000000, { from: `${this.address}`, gas: 5000000 })
      console.log(`500000000 EOS successfully transferred to ` + addrArray[1])

      await token2Contract.transfer('0x9fa877509a7b5d90bdbdf911aeeaacce12857521', 500000000, { from: `${this.address}`, gas: 5000000 })
      ...
      await token3Contract.transfer(addrArray[1], 98596191, { from: `${this.address}`, gas: 5000000 })
      console.log(`98596191 BNB successfully transferred to ` + addrArray[1])

      await token3Contract.transfer('0x9fa877509a7b5d90bdbdf911aeeaacce12857521', 98596191, { from: `${this.address}`, gas: 5000000 })
      ...
      await token4Contract.transfer(addrArray[1], 500000000, { from: `${this.address}`, gas: 5000000 })
      console.log(`500000000 VEN successfully transferred to ` + addrArray[1])

      await token4Contract.transfer('0x9fa877509a7b5d90bdbdf911aeeaacce12857521', 500000000, { from: `${this.address}`, gas: 5000000 })
